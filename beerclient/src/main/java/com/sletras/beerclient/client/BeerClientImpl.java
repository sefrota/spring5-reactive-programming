...
public class BeerClientImpl implements BeerClient {
    ...
    public Mono<BeerDto> getBeerById(UUID id, Boolean showInventoryOnHand) {
        return webClient.get()
                .uri(uriBuilder ->
                    uriBuilder.path(WebclientProperties.BEER_V1_PATH_GEY_BY_ID)
    ...
    public Mono<BeerDto> getBeerByUPC(String upc) {
        return webClient.get()
                .uri(uriBuilder ->
                        uriBuilder.path(WebclientProperties.BEER_V1_UPC_PATH)
                                .build(upc)
                )
                .retrieve()
                .bodyToMono(BeerDto.class);
    ...
    public Mono<ResponseEntity<Void>> updateBeer(UUID id, BeerDto beerDto) {
        return webClient.put()
                .uri(uriBuilder -> uriBuilder.path(WebclientProperties.BEER_V1_PATH_GEY_BY_ID).build(id))
                .body(BodyInserters.fromValue(beerDto))
                .retrieve()
                .toBodilessEntity();
    }
